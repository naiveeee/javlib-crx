...
export default function (id) {
  ...
  return axios({
    ...
    const nodeList = doc.querySelectorAll('#list .tmb a')
    // 例： （数字或无）abp（数字或无）001（字母或无）
    const reg = new RegExp(`^\\d*${id.split('-')[0]}\\d*${id.split('-')[1]}\\D*$`, 'i')
    // 匹配第一个符合条件的cid
    for (let i = 0; i < nodeList.length; i++) {
      ...
      if (reg.test(cid)) {
        return axios({
          ...
          const _doc = parser.parseFromString(resp.data, 'text/html')
          const imgList = _doc.querySelectorAll('#sample-image-block a img')
          if (!imgList.length) {
